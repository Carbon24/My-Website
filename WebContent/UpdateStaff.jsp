...
/* int rs=st.executeUpdate("update staff set emp_salary=1.1*emp_salary where emp_id='regid'");
if(rs!=0)System.out.println("SUCCESS");
else System.out.println("ooppppppssssss");
 */
String empid = request.getParameter("regid"); 
String s = request.getParameter("sal");
if (empid != null) {
	String query = "UPDATE STAFF SET EMP_SALARY = " + s + " WHERE EMP_ID = " + empid; 
	int rs = st.executeUpdate(query);
	if (rs != 0) {
		out.println("<h2>THE SALARY FOR EMPLOYEE WITH EMPID : " + empid + " IS UPDATED !!"); 
	} else {
